...
function renderCombine(){
  ...
  return `
    <div class="card nudge-card">
      <span class="nudge-tag">WE NOTICED</span>
      <h3>You're running 3 separate Altura connections at this address.</h3>
      <p>Mobile, Home Wi-Fi and TV are billed apart today. Combine them under one Altura Everyday plan — same total, one bill, one app. Never a discount trick, always the same price.</p>
...
function renderOttTiles(){
  ...
  return list.map(p => `
    <div class="ott-tile ${p.unused ? 'unused' : ''}">
      <div class="logo" style="background:${p.have ? 'rgba(232,17,45,0.14)' : 'rgba(255,255,255,0.06)'}; color:${p.have ? 'var(--red)' : 'var(--grey)'};">${p.name.slice(0,2).toUpperCase()}</div>
      <div class="name">${p.name}</div>
      <div class="meta">${p.cat}</div>
      ${p.have
        ? `<span class="ott-tag ${p.unused ? 'unused' : 'have'}">${p.unused ? 'Included \u2014 unopened 30d' : 'Included'}</span>`
        : `<span class="ott-tag" style="background:rgba(255,255,255,0.06); color:var(--grey);">Not included</span>`
      }
    </div>
  `).join('');
}
